...
4. **占位符**（深层校验，原则：宁可漏检不可误报）：
   - `${}` / `@{}` / `%{}` / `&{}` **括号闭合**（未闭合报错）。
   - `@{}` 引用的实体必须存在于实体池（跨文件），否则报错。
   - `%{}`（运行时上下文变量）/ `&{}`（动态表达式）跳过，运行时确定。
   - `${}` **不做存在性硬校验**：`${option}` 短格式由运行时在保存时创建 `{当前实体key}-values` section；`${section:option}` 长格式的 section 若在实体池中视为合法，不在实体池中则跳过（配置值 section 不都在实体池，避免误报）。
