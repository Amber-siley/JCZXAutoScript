...
`action` 在此类型中**不是执行链**，而是对匹配区域进行位置/尺寸变换的操作序列。多个操作用逗号分隔，依次应用到匹配结果上。所有操作使用 `|` 作为分隔符（与 `&{...}` 上下文运算一致）：
...
> `N` 为数字（整数或浮点数，如 `1.5`、`0.8`、`30`、`-20`）。转换类操作以模板尺寸为基数，边移动类以像素为单位。变换在匹配结果的四个角点和中心点上同步生效。
...
> 整体平移与单边移动可混合使用（按 `action` 列表顺序依次生效），但不能与缩放混合（后执行覆盖前者）。推荐在缩放后另开一个 `match` 实体做边调整。
